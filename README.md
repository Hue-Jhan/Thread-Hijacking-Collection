...
First the payload is decrypted, for a better view of the encryption check out the encoding mechanism [here](https://github.com/Hue-Jhan/Simple-shellcode-crypter), then the code takes a handle to the target process, and finds a thread inside it. Once a thread is found/created, the program will allocate memory for the payload, upload the shellcode in it, and change its permissions to RWX.
...
- We do not iterate through every thread but simply create an already suspended one with ```CreateRemoteThread()```, in order for it to open successfully we point the base address to a dummy function that does nothing;
- when hijacking the thread, we dont suspend it because it's already suspended.

This version is more likely to be detected because it uses a more detectable injection pattern: creating a new thread after allocating some memory, copying some freshly decrypted piece of code in it, and making it executable. The process at least won't crash because we did not hijack one of its threads.
